...
def load_dataset(flatten=False):
    ...
    # normalize x
    X_train = X_train.astype(float) / 255.X
    ...
    return XX_train, y_train, X_val, y_val, X_test, y_test
...
def forward(network, X):
    ...
    return aXs (N,784), i.e. a list of flattened MNIST images
...
def iterate_minibatches(inputs, targets, batchsize, shuffle=False):
    ...
    for start_idx in trange(0, len(inputs) - batchsize + 1, batchsize):
        if shuffle:
            ...
            excerpt = slice(start_idx, start_idx + batchsize) 5.989 
